...
build_and_upload_image () {
	if [ -d ./deploy/${image_dir_name} ] ; then
		...
		if [ -f ${image_name}-${size}.img ] ; then
			...
			#upload:
			 ssh ${ssh_user} mkdir -p ${server_dir}/respeaker/test/debian/${time}
			 rsync -e ssh -avh ./${image_name}-${size}.bmap ${ssh_user}:${server_dir}/respeaker/test/debian/${time}
			 rsync -e ssh -avh ./${image_name}-${size}.img.xz ${ssh_user}:${server_dir}/respeaker/test/debian/${time}
			 rsync -e ssh -avh ./${image_name}-${size}.img.xz.sha256sum ${ssh_user}:${server_dir}/respeaker/test/debian/{time}
